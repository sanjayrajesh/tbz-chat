...
use tbz_chat_backend_rs::invitation::{InvitationServiceImpl};
use tbz_chat_backend_rs::mail::MailService;
use tbz_chat_backend_rs::swagger;
use tbz_chat_backend_rs::user;
use tbz_chat_backend_rs::user::{UserRepositoryImpl, UserServiceImpl, UserService};
use tbz_chat_backend_rs::verification_token::{
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .service(web::scope("/swagger-ui").configure(swagger::swagger(spec, config)))
