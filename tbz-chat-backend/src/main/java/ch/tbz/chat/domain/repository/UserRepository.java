...
public interface UserRepository extends DomainEntityRepository<User> {
  ...
  @Query(
      "from User u left join UserInChat uic on uic.user.id = u.id where ((:excludedUserId is null and u.id is not null) or u.id <> :excludedUserId) and ((:excludeChatId is null and uic.chat.id is not null) or uic.chat.id <> :excludeChatId) and (lower(u.email) like concat('%', :query, '%') or lower(u.username) like concat('%', :query, '%')) and :query is not null")
